...
class MediasListContainer extends Component {
  ...
  renderDeleteMediaButton = (media) => {
    ...
    if(ability('media-delete')){
      return <DeleteModal buttonProps={{block:true}} model="Media" delete={()=>this.deleteMedia(media.id)}/>;
    }
  };
              ...
              <Row>
                ...
                <Col sm="9">
                  <div style={{zIndex:"150",position:'relative'}}>
                    {/* <RestaurantSelect autoSelect  {...this.props} onChange={(nook_id => this.setState({nook_id}))}/> */}
                  </div>
                </Col>
              </Row>
            </CardHeader>
            <CardBody>
              <Medias {...this.props} nook_id={nook_id} style={{height: '600px'}} upBtnSize='lg' imgColSize="3" onChange={media => this.setState({media})}/>
            </CardBody>
          </Card>
        </Col>
        <Col xs="3">
