...
class EditMedia extends Component {
  ...
  edit = () => {
    ...
    let { id, name, caption, alt, nook_id, image, } = this.state;
  ...
  renderEditMediaButton = () => {
    ...
    if (ability('media-edit')) {
      return (
        <Col xs="12">
          <Button color="primary" onClick={this.edit} disabled={processing} block>
        ...
        <FormGroup>
          <Row className='uploadArea' onClick={() => this.fInput.click()}>
            ...
            <Col sm={{ size: 6, offset: 3 }} className="text-center">
              <Button block color="success" size="sm" className="btn-pill mt-3"> <i className="fa fa-upload" /> Upload</Button>
