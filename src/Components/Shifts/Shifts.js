...
class Shifts extends Component {
  ...
  getShifts = search => {
    ...
    let { getShifts, dispatch, user, alertify } = this.props;

    getShifts(user.accessToken, search)
      .then(res => {
        this.setState({
  ...
  next = () => {
    ...
    if (next <= this.state.totalPages) {
      ...
      this.getShifts(search + "page=" + next);
    }
  };
  ...
  previous = () => {
    ...
    if (previous > 0) {
      let search = getSearchUrlFromState(this.state);
      this.getShifts(search + "page=" + previous);
    }
  };
  ...
  renderBody = () => {
    ...
    return this.props.shifts.map(m => {
      return (
        <tr key={m.id}>
          <td>{m.id}</td>
          ...
          <Col xs="12" lg="12">
            <Card>
              <CardHeader>
                <Row>
                  <Col md="2">
                    <i className="fa fa-align-justify" /> Shifts - Page {page}{" "}
                    of {totalPages}
                  </Col>
...
const mapDispatchToProps = () => {
  return {
    getShifts: (token, search) => actions.getShifts(token, search)
  };
};
